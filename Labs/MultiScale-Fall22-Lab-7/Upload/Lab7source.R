...
mouseChr3 <- seqinr::read.fasta("Homo_sapiens_Chr3.fasta")[[1]]
print(paste0("Chromosome sequence successfully loaded | ", date()))

write.csv(x = ORFTable(mouseChr3, 100), file = "./Homo_sapiens_Chr3_ORFs_forward.csv", row.names = F)
print(paste0("Forward ORFs written out | ", date()))

write.csv(x = ORFTable(compStrand(mouseChr3), 100), file = "./Homo_sapiens_Chr3_ORFs_reverse.csv", row.names = F)
